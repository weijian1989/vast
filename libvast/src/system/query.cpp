...
void ship_results(stateful_actor<query_state>* self) {
...
void shutdown(stateful_actor<query_state>* self) {
  ...
  if (self->state.accountant) {
    ...
    self->send(self->state.accountant, "query.hits", hits);
    self->send(self->state.accountant, "query.processed", processed);
    self->send(self->state.accountant, "query.results", results);
    self->send(self->state.accountant, "query.shipped", shipped);
    self->send(self->state.accountant, "query.selectivity", selectivity);
    self->send(self->state.accountant, "query.runtime", runtime);
  }
  self->send_exit(self, exit_reason::normal);
}

void request_more_hits(stateful_actor<query_state>* self) {
...
behavior query(stateful_actor<query_state>* self, expression expr,
                  query_options) {
    ...
    [=](bitmap& hits) {
      ...
      if (self->state.accountant) {
        if (self->state.hits.empty())
          self->send(self->state.accountant, "query.hits.first", runtime);
        self->send(self->state.accountant, "query.hits.arrived", runtime);
        self->send(self->state.accountant, "query.hits.count", count);
      }
      ...
      if (self->state.stats.received < self->state.stats.expected) {
        ...
        VAST_DEBUG(self, "received all", self->state.stats.expected,
                   "bitmap(s) in", runtime);
        if (self->state.accountant)
          self->send(self->state.accountant, "query.hits.runtime", runtime);
        shutdown(self);
      }
    },
